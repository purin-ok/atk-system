...
int main(int argc, char **argv) {
    int t;
    double amp,frq,phf,rad,vin,esum=0; /* esumは演習2-3,phfは演習2-6で使用 */
    ...
    frq = atof(argv[2]);
    printf("#A %f\n", amp); /* サンプリング情報を出力 */
    printf("#F %f\n", frq);//#をつけておくとgnuplotの出力には影響しないけど、人間は見ることはできる
    printf("#T %f\n", DT);
    printf("#B %f\n", A_BIAS);
    printf("#N %f\n", T_END/DT + 1);

    for (t = 0; t <= T_END; t += DT) {
        rad = t / (1000/frq) * 2 * PI; /* 時刻t[ms]を弧度に変換する式を考えること（式を必ず報告すること） */
...
//[0,255]超えたらラインの値にすりゃいいんじゃね | 波形が超越した．理由:vinの値がunsigned charで表せる値を飛び越えたけど，
